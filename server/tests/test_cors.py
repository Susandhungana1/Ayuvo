...
The frontend is ayuvo-health.vercel.app (legacy medistore-health.vercel.app
kept as alias). The backend accepts its own Vercel project's domains via a
regex (in addition to the CORS_ORIGINS env allowlist), so a rename/redeploy
never silently breaks API calls — while still rejecting arbitrary third-party
origins.
"""
...
def test_regex_allows_this_projects_vercel_domains():
    assert _pat.match("https://ayuvo-health.vercel.app")
    assert _pat.match("https://front-ayuvo-app.vercel.app")
    ...
    assert _pat.match("https://ayuvo-share-beige.vercel.app")
    assert _pat.match("https://ayuvo-share.vercel.app")
    # Legacy medistore-* aliases kept during cutover.
...
def test_regex_rejects_foreign_and_malformed_origins():
    ...
    assert not _pat.match("https://ayuvo-health.vercel.app.evil.com")
    assert not _pat.match("https://ayuvo-share-beige.vercel.app.evil.com")
    assert not _pat.match("https://ayuvo-share-other.com")  # not vercel.app
    ...
    assert not _pat.match("http://ayuvo-health.vercel.app")  # not https
    assert not _pat.match("https://ayuvo-health.vercel.app/")  # trailing slash
